...
-- findM p =
--   foldRight (\a foa -> 
--     lift2 (\b oa -> if b then Full a else oa) (p a) foa
--   ) (pure Empty)
...
isHappy ::
  ...
  -> Bool
-- isHappy nx=
--   let ssd n = toInteger $ sum (square . digitToInt  <$> show' n)
--       xs = produce ssd x
--       repeated = firstRepeat xs
--   in contains 1 repeated
isHappy =
  contains 1 .
    firstRepeat .
    produce(\n ->
